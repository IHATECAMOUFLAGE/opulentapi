(function(){
  ...
  function proxyUrl(url){
    ...
    try{ return '/api/proxy?url='+encodeURIComponent(new URL(url, location.href).toString()); } catch { return url; }
  }

  function rewriteElement(el){
    if(el.tagName==='A' && el.href) el.href=proxyUrl(el.href);
    if(el.tagName==='IMG' && el.src) el.src=proxyUrl(el.src);
    if(el.tagName==='IFRAME' && el.src) el.src=proxyUrl(el.src);
    if(el.tagName==='LINK' && el.href) el.href=proxyUrl(el.href);
    if(el.tagName==='SCRIPT' && el.src) el.src=proxyUrl(el.src);
    if(el.tagName==='FORM' && el.action) el.action=proxyUrl(el.action);
    const style = el.style;
    if(style){
      for(const k in style){
        if(style[k] && typeof style[k]==='string' && style[k].includes('url(')){
          style[k] = style[k].replace(/url\(["']?([^"')]+)["']?\)/g,(m,u)=>`url(${proxyUrl(u)})`);
        }
      }
    }
  }
  ...
  function rewriteAll(){
    document.querySelectorAll('a,img,iframe,link,script,form,[style]').forEach(rewriteElement);
    document.querySelectorAll('*').forEach(el=>{
      if(el.onclick) el.onclick = new Function('event', 'return ('+el.onclick.toString().replace(/(window\.location|window\.top\.location|location)\.href\s*=\s*["\']([^"\']+)["\']/g,(m,obj,url)=>`${obj}.href="${proxyUrl(url)}"`)+')(event);');
    });
  }
  ...
  observer2.observe(document.documentElement||document.body||document,{childList:true,subtree:true});
