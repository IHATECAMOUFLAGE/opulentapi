(function(){
  ...
  const observer=new MutationObserver(mutations=>{
    mutations.forEach(m=>{
      m.addedNodes.forEach(n=>{
        ...
        ['src','href','poster','data-src','data-href','srcset'].forEach(attr=>{
          if(n.hasAttribute && n.hasAttribute(attr)){
            n.setAttribute(attr, rewriteUrl(n.getAttribute(attr)));
          }
        });
        if(n.onclick) n.onclick=new Function('event', rewriteUrl(n.onclick.toString()));
      });
    });
  });
  observer.observe(document.documentElement||document.body||document,{childList:true,subtree:true});

  document.querySelectorAll('a[href]').forEach(a=>a.href=rewriteUrl(a.getAttribute('href')));
  document.querySelectorAll('button[onclick]').forEach(b=>{
    const orig=b.getAttribute('onclick');
    b.setAttribute('onclick', orig.replace(/window\.location(\.href)?\s*=\s*['"]([^'"]+)['"]/gi,(m,p1,url)=>{
  ...
  const loc=Object.getOwnPropertyDescriptor(window.location.__proto__,'href');
  if(loc && loc.configurable){
    Object.defineProperty(window.location,'href',{
      set:function(val){ window.location.assign(rewriteUrl(val)); },
      get:function(){ return loc.get.call(window.location); }
    });
  }

  document.querySelectorAll('form[action]').forEach(f=>f.action=rewriteUrl(f.action));
  ...
  function createMenu(){
    ...
    erudaCheckbox.onchange=()=>{
      ...
      else if(window.eruda){window.eruda.destroy(); window.eruda=null; erudaLoaded=false;}
    };
