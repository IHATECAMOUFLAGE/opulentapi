(function(){
  ...
  function removeEruda(){
    if(window.eruda){
      eruda.destroy();
      const scripts=document.querySelectorAll('script[src*="eruda"]');
  ...
  var observer=new MutationObserver(mutations=>{
    mutations.forEach(m=>{
      m.addedNodes.forEach(n=>{
        if(n.nodeType===1){
          ['src','href','poster','data-src','data-href','srcset','action','formaction'].forEach(a=>{
  ...
  function rewriteElement(el){
    if(!el) return;
    ['href','src','srcset','poster','action','formaction'].forEach(attr=>{
      if(el.hasAttribute && el.hasAttribute(attr)){
        const val=el.getAttribute(attr);
        if(val && !val.startsWith('/api/proxy') && !val.startsWith('javascript:') && !val.startsWith('data:')){
          try{
            const absolute=new URL(val, location.href).toString();
    ...
    if(el.style){
      el.style.cssText=el.style.cssText.replace(/url\(([^)]+)\)/gi,(m,u)=>{
        u=u.replace(/['"]/g,'');
        if(u.startsWith('data:')||u.startsWith('/api/proxy')||u.startsWith('javascript:')) return m;
        try{ return 'url("/api/proxy?url='+encodeURIComponent(new URL(u,location.href).toString())+'")'; }catch{return m;}
      });
    }
  }
